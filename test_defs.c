...
Status test_gvega_destroy_sets_object_to_null(char* buffer, int length)
{
    ...
    {
        status = Success;
        strncpy(buffer, "test_gvega_destroy_sets_object_to_null\n"
            , length);
    }
...
Status test_gvega_init_c_string_sets_the_correct_string(char* buffer, int length)
{
    ...
    for (int i = 0; i < my_string_get_size(hString); i++) {
        if (*my_string_at(hString, i) != c[i])
        {
            ...
            strncpy(buffer, "test_gvega_init_c_string_sets_the_correct_string\n", length);
        }
        else
        {
            status = Success;
...
Status test_gvega_init_c_string_sets_correct_size(char* buffer, int length)
{
    ...
    if (my_string_get_size(hString) != strlen(c))
    {
        ...
        strncpy(buffer, "test_gvega_init_c_string_sets_correct_size\n", length);
    }
    else
    {
        status = Success;
...
Status test_gvega_init_c_string_sets_correct_capacity(char* buffer, int length)
{
    ...
    if (my_string_get_capacity(hString) != strlen(c))
    {
        status = Failure;
        printf("expected the number %d but got %d\n", strlen(c), my_string_get_size(hString));
        strncpy(buffer, "test_gvega_init_c_string_sets_correct_capacity\n", length);
    }
    else
    {
        status = Success;
...
Status test_gvega_get_capacity_returns_correct_number(char* buffer, int length)
{
    ...
    if (my_string_get_capacity(hString) != strlen(c))
    {
        status = Failure;
        printf("expected the number %d but got %d\n", strlen(c), my_string_get_size(hString));
        strncpy(buffer, "test_gvega_get_capacity_returns_correct_number\n", length);
    }
    else
    {
        status = Success;
...
Status test_gvega_get_size_returns_correct_number(char* buffer, int length)
{
    ...
    if (my_string_get_size(hString) != strlen(c))
    {
        status = Failure;
        printf("expected the number %d but got %d\n", strlen(c), my_string_get_size(hString));
        strncpy(buffer, "test_gvega_get_size_returns_correct_number\n", length);
    }
    else
    {
        status = Success;
...
Status test_gvega_compare_same_string_returns_0(char* buffer, int length)
{
    ...
    if (my_string_compare(hString, hString) != 0)
    {
        ...
        strncpy(buffer, "test_gvega_compare_same_string_returns_0\n", length);
    }
    else
    {
        status = Success;
...
Status test_gvega_compare_different_string_returns_not0(char* buffer, int length)
{
    ...
    if (my_string_compare(hString, hString0) == 0)
    {
        ...
        strncpy(buffer, "test_gvega_compare_different_string_returns_not0\n", length);
    }
    else
    {
        status = Success;
...
Status test_gvega_extraction_sets_correct_string(char* buffer, int length)
{
    ...
    for (int i = 0; i < my_string_get_size(hString); i++) {
        if (*my_string_at(hString, i) != c[i])
        {
            status = Failure;
            printf("expected the char %c but got char %c\n", c[i], *my_string_at(hString, i));
            strncpy(buffer, "test_gvega_extraction_sets_correct_string\n", length);
        }
        else
        {
            status = Success;
...
Status test_gvega_extraction_sets_correct_size(char* buffer, int length)
{
    ...
    if (my_string_get_size(hString) != strlen(c))
    {
        status = Failure;
        printf("expected the number %d but got %d\n", strlen(c), my_string_get_size(hString));
        strncpy(buffer, "test_gvega_extraction_sets_correct_size\n", length);
    }
    else
    {
        status = Success;
...
Status test_gvega_insertion_inserts_correct_string(char* buffer, int length)
{
    ...
    for (int i = 0; i < my_string_get_size(hString0); i++) {
        if (*my_string_at(hString, i) != *my_string_at(hString0, i))
        {
            ...
            strncpy(buffer, "test_gvega_insertion_inserts_correct_string\n", length);
        }
        else
        {
            status = Success;
...
Status test_gvega_push_back_sets_correct_short_string(char* buffer, int length) {
    ...
    for (int i = 0; i < my_string_get_size(hString); i++) {
        if (*my_string_at(hString, i) != c[i])
        {
            status = Failure;
            printf("expected the char %c but got char %c\n", c[i], *my_string_at(hString, i));
            strncpy(buffer, "test_gvega_push_back_sets_correct_short_string\n", length);
        }
        else
        {
            status = Success;
...
Status test_gvega_push_back_sets_correct_long_string(char* buffer, int length) {
    ...
    for (int i = 0; i < my_string_get_size(hString); i++) {
        if (*my_string_at(hString, i) != c[i])
        {
            status = Failure;
            printf("expected the char %c but got char %c\n", c[i], *my_string_at(hString, i));
            strncpy(buffer, "test_gvega_push_back_sets_correct_long_string\n", length);
        }
        else
        {
            status = Success;
...
Status test_gvega_push_back_sets_correct_size_short(char* buffer, int length) {
    ...
    if (my_string_get_size(hString) != strlen(c))
    {
        ...
        strncpy(buffer, "test_gvega_push_back_sets_correct_size_short\n", length);
    }
    else
    {
        status = Success;
...
Status test_gvega_push_back_sets_correct_capacity_long(char* buffer, int length) {
    ...
    if (my_string_get_capacity(hString) != (capacity * 2))
    {
        status = Failure;
        printf("expected the num %d but got %d\n", strlen(c), my_string_get_size(hString));
        strncpy(buffer, "test_gvega_push_back_sets_correct_capacity_long\n", length);
    }
    else
    {
        status = Success;
...
Status test_gvega_pop_back_sets_correct_size(char* buffer, int length) {
    ...
    if (my_string_get_size(hString) != (size - 5))
    {
        status = Failure;
        printf("expected the num %d but got %d\n", strlen(c), my_string_get_size(hString));
        strncpy(buffer, "test_gvega_pop_back_sets_correct_size\n", length);
    }
    else
    {
        status = Success;
...
Status test_gvega_concat_sets_right_string(char* buffer, int length)
{
    ...
    for (int i = 0; i < strlen(c); i++) {
        if (*my_string_at(hString, i) != c[i])
        {
            status = Failure;
            printf("expected the char %c but got char %c\n", *my_string_at(hString, i), *my_string_at(hString0, i));
            strncpy(buffer, "test_gvega_concat_sets_right_string\n", length);
        }
        else
        {
            status = Success;
...
Status test_gvega_concat_sets_right_size(char* buffer, int length)
{
    ...
    if (my_string_get_size(hString) != strlen(c))
    {
        status = Failure;
        printf("expected the num %d but got %d\n", strlen(c), my_string_get_size(hString));
        strncpy(buffer, "test_gvega_concat_sets_right_size\n", length);
    }
    else
    {
        status = Success;
...
Status test_gvega_at_returns_right_adress(char* buffer, int length)
{
    ...
    for (int i = 0; i < my_string_get_size(hString); i++) {
        if (*my_string_at(hString, i) != c[i])
        {
            ...
            strncpy(buffer, "test_gvega_at_returns_right_adress\n", length);
        }
        else
        {
            status = Success;
...
Status test_gvega_c_str_returns_right_adress(char* buffer, int length)
{
    ...
    for (int i = 0; i < my_string_get_size(hString); i++) {
        if (my_string_c_str(hString)[i] != c[i])
        {
            status = Failure;
            printf("expected the char %c but got %c\n", c[i], *my_string_at(hString, i));
            strncpy(buffer, "test_gvega_c_str_returns_right_adress\n", length);
        }
        else
        {
            status = Success;
...
Status test_gvega_empty_returns_false(char* buffer, int length)
{
    ...
    if (my_string_empty(hString) != FALSE)
    {
        ...
        strncpy(buffer, "test_gvega_empty_returns_false\n", length);
    }
    else
    {
        status = Success;
...
Status test_gvega_empty_returns_true(char* buffer, int length)
{
    ...
    if (my_string_empty(hString) != TRUE)
    {
        ...
        strncpy(buffer, "test_gvega_empty_returns_true\n", length);
    }
    else
    {
        status = Success;
